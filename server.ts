...
Use Reel only for paper reel/roll items with reel, roll, kraft, paper, GSM, BF, deckle, or size indicators. For Reel lines, if the item text contains patterns like Size 178 / GSM 140 / BF 18 or 99 CM X GSM: 120 X BF: 18, populate numeric size, gsm, and bf exactly from that text. For Reel color, extract only the original color printed on the invoice or packing slip, and only when it is exactly one of these values: NS, Natural, LG, Duplex. Search item descriptions, color fields, PO/order text, and packing slip rows for those exact tokens. Do not infer, translate, map, or suggest colors from words like kraft, brown, white, or any unsupported code. If NS, Natural, LG, or Duplex is not explicitly visible, return color as blank. For Reel lines, extract every visible individual reel row under that item/spec into reels with supplierReelNo and weightKg; include supplierPoNo per reel when visible, otherwise blank. Keep qty as the total invoice weight for the item/spec. If individual reel rows are not visible, use reels: []. Use Other for consumables, spares, chemicals, packing material, and non-reel goods. Dates must be ISO YYYY-MM-DD when visible. Numbers must be numeric. If unsure, leave fields blank or null and lower confidence.`
  ...
  try {
    ...
    return res.status(500).json({ error: (error as Error).message || "Failed to extract invoice data." });
  }
});
...
function formatDispatchPlanNo(fy: string, sequence: number) {
  return `DP/${fy}/${String(sequence).padStart(5, "0")}`;
}

async function migrateLegacyDispatchPlanNos(db: mysql.Pool) {
  const [rows] = await db.query("SELECT `id`, `planNo`, `date` FROM `dispatch_plans` WHERE `planNo` REGEXP '^DP-[0-9]+$'");
  let migrated = 0;
  let skipped = 0;

  for (const row of rows as Array<{ id: string; planNo?: string; date?: string }>) {
...
async function buildMaterialValuationMaps(db: mysql.Pool) {
  ...
  const npdRateByMaterialId = new Map<string, number>();
  ...
  for (const item of npdRows as any[]) {
    const rate = roundCurrency(Number(item.rate || 0));
    if (rate <= 0) continue;
  ...
  `);
  const receiptRateByPackingSlip = new Map<string, number>();
...
function resolveBackfillMaterialRate(
  materialId: string,
  maps: Awaited<ReturnType<typeof buildMaterialValuationMaps>>
) {
...
async function backfillNonJobMaterialIssueValuation(db: mysql.Pool) {
  ...
  const getDateTime = (value: unknown, endOfDay = false) => {
  ...
  const [materialInRows] = await db.query("SELECT id, date, timestamp, `lines` FROM `material_in`");
  const purchaseRatesByMaterial = new Map<string, Array<{ rate: number; receiptDateTime: number; sortTime: number }>>();
  for (const receipt of materialInRows as any[]) {
  ...
  const getLatestPurchaseRateAsOf = (materialId: string, issueDate: unknown) => {
    const issueDateTime = getDateTime(issueDate, true);
    if (!issueDateTime) return 0;
    return (
      (purchaseRatesByMaterial.get(materialId) || [])
        .filter((entry) => entry.receiptDateTime <= issueDateTime)
        .sort((a, b) => b.sortTime - a.sortTime)[0]?.rate || 0
    );
  };
...
async function backfillOldJobMaterialIssueReturnValuation(db: mysql.Pool) {
  ...
  for (const reelLine of issueReelRows as any[]) {
  ...
  for (const line of issueLineRows as any[]) {
  ...
  const [jobIssueValueRows] = await db.query(`
    ...
    WHERE LOWER(TRIM(COALESCE(mi.issueType, ''))) = 'job'
  `);
  const issueValueByJobMaterial = new Map<string, { qty: number; amount: number }>();
  for (const line of jobIssueValueRows as any[]) {
    const materialId = String(line.materialId || "").trim();
  ...
  for (const reelLine of returnReelRows as any[]) {
  ...
  for (const line of returnLineRows as any[]) {
    const lineId = String(line.id || "");
    const materialId = String(line.materialId || "").trim();
    const qty = roundCurrency(Number(line.qty || 0));
    const existingLastPurchaseRate = roundCurrency(Number(line.lastPurchaseRate || 0));
    ...
    const issueValue =
      (productionId ? issueValueByJobMaterial.get(`pid:${productionId}::${materialId}`) : undefined) ||
      (jobNo ? issueValueByJobMaterial.get(`job:${jobNo}::${materialId}`) : undefined);
...
type TruckStatusUpdateSource = "System" | "TruckDriver" | "PublicDriver" | "AppVehicleUpdate";
...
await db.query(`
      ...
      const migrations = [
        ...
        { table: "material_in", column: "tdsAmount", type: "DECIMAL(15, 2) NOT NULL DEFAULT 0" },
      ...
      try {
        ...
        console.warn("[DB] Could not backfill old job material issue/return valuation:", (err as Error).message);
      }
...
function getPhysicalStockFinancialYear(date = new Date()) {
  const year = date.getFullYear();
...
async function fetchOpenPhysicalStockSession(db: mysql.Pool | mysql.PoolConnection) {
  const [rows] = await db.query(
    "SELECT * FROM `physical_stock_sessions` WHERE LOWER(TRIM(`status`)) = 'open' ORDER BY `startedAt` DESC LIMIT 1"
  );
  return ((rows as any[])[0] || null) as PhysicalStockSessionRow | null;
}

async function generatePhysicalStockSessionNo(db: mysql.Pool | mysql.PoolConnection, fy: string) {
  const prefix = `PS/${fy}/`;
  const [rows] = await db.query(
  ...
  const lastNo = String((rows as any[])[0]?.sessionNo || "");
...
async function requirePhysicalStockAccess(req: express.Request, res: express.Response, next: express.NextFunction) {
  try {
    const user = await getRequestUser(req);
    if (!user) return res.status(401).json({ error: "Unauthorized" });
...
app.post("/api/physical-stock/start-session", requireAuth, requirePhysicalStockAccess, async (req, res) => {
  ...
  const fy = getPhysicalStockFinancialYear(new Date());
  const conn = await db.getConnection();
  ...
  try {
    ...
    const session: PhysicalStockSessionRow = {
...
app.post("/api/physical-stock/restart-session", requireAuth, requirePhysicalStockAccess, async (req, res) => {
  try {
    ...
    const session = (sessionRows as any[])[0] as PhysicalStockSessionRow | undefined;
...
function mapPublicDriverStatus(raw: unknown) {
  const status = String(raw || "").trim().toUpperCase();
...
type GlobalItemTransferModule = "orders" | "plans" | "dispatch" | "loading" | "billing";

type GlobalItemTransferIdentity = {
  id: string;
  source: GlobalItemRenameSource;
  name: string;
  erp: string;
};

const GLOBAL_ITEM_TRANSFER_MODULE_LABELS: Record<GlobalItemTransferModule, string> = {
  orders: "Orders",
...
const GLOBAL_ITEM_TRANSFER_MODULES = new Set<GlobalItemTransferModule>([
  "orders",
  "plans",
  "dispatch",
  "loading",
...
function normalizeGlobalItemMaintenanceSource(value: unknown): GlobalItemRenameSource | null {
  const source = String(value || "").trim().toUpperCase();
  const normalized = source === "NPD" ? "FG" : source;
  return GLOBAL_ITEM_RENAME_SOURCE_CONFIG[normalized as GlobalItemRenameSource]
    ? (normalized as GlobalItemRenameSource)
    : null;
}

function normalizeGlobalItemTransferModules(value: unknown): GlobalItemTransferModule[] {
  if (!Array.isArray(value)) return [];
  return Array.from(
    new Set(
      value
        .map((moduleName) => String(moduleName || "").trim().toLowerCase() as GlobalItemTransferModule)
        .filter((moduleName) => GLOBAL_ITEM_TRANSFER_MODULES.has(moduleName))
    )
  );
}

function getItemReferenceCondition(alias: string, source: GlobalItemRenameSource) {
  const prefix = alias ? `${alias}.` : "";
  if (source === "FG") {
    return {
      sql: `((COALESCE(NULLIF(TRIM(${prefix}\`itemSource\`), ''), 'FG') = 'FG' AND ${prefix}\`itemId\` = ?) OR ${prefix}\`npdId\` = ?)`,
      buildParams: (itemId: string) => [itemId, itemId],
    };
  }

  return {
    sql: `(${prefix}\`itemId\` = ? AND UPPER(COALESCE(NULLIF(TRIM(${prefix}\`itemSource\`), ''), 'FG')) = ?)`,
    buildParams: (itemId: string) => [itemId, source],
  };
}

function addAuditUpdateParts(
  columns: Set<string>,
  updateParts: string[],
  params: any[],
  actor: string,
...
async function getGlobalItemTransferIdentity(
  db: any,
  database: string,
  source: GlobalItemRenameSource,
  itemId: string
): Promise<GlobalItemTransferIdentity | null> {
  const config = GLOBAL_ITEM_RENAME_SOURCE_CONFIG[source];
  const columns = await getExistingColumnNames(db as mysql.Pool, database, config.tableName);
  ...
  const erpExpr = erpColumns.length
    ? `COALESCE(${erpColumns.map((column) => `NULLIF(TRIM(\`${column}\`), '')`).join(", ")}, '')`
    : "''";
...
function normalizeLoadingLineSource(value: unknown): GlobalItemRenameSource {
  return normalizeGlobalItemMaintenanceSource(value) || "FG";
}

function parseJsonArrayField(value: unknown): any[] {
  if (Array.isArray(value)) return value;
  if (typeof value !== "string") return [];
...
function transferLinkedItemInArray(
  rows: any[],
  source: GlobalItemRenameSource,
  fromItemId: string,
  toItem: GlobalItemTransferIdentity
) {
  let changed = false;
  let lineCount = 0;
  ...
  const nextRows = rows.map((row) => {
    ...
    const rowSource = normalizeLoadingLineSource(row.itemSource || (row.source as unknown));
...
async function collectLoadingTransferRows(
  db: any,
  database: string,
  source: GlobalItemRenameSource,
  fromItemId: string,
  ...
  let records = 0;
  let lines = 0;
  const tableCounts: Record<string, { records: number; lines: number }> = {};
  const detailRows: any[] = [];

  for (const tableName of loadingTables) {
    const columns = await getExistingColumnNames(db as mysql.Pool, database, tableName);
    ...
    for (const row of rows as any[]) {
      ...
      if (lineTransfer.changed) {
        ...
        originalLines.forEach((line) => {
          ...
          detailRows.push({
            ...
            jobNos: Array.isArray(line?.jobNos) ? line.jobNos.map((job: any) => String(job)).join(", ") : "",
      ...
      if (toItem && updateParts.length > 0) {
        addAuditUpdateParts(columns, updateParts, params, actor || "System User", now || new Date().toISOString());
        params.push(row.id);
...
async function countGlobalItemTransfer(
  db: any,
  database: string,
  source: GlobalItemRenameSource,
  fromItemId: string,
  modules: GlobalItemTransferModule[]
) {
  const counts: Record<GlobalItemTransferModule, number> = {
    orders: 0,
    plans: 0,
    dispatch: 0,
    loading: 0,
  ...
  if (modules.includes("orders")) {
    ...
    details.orders = (rows as any[]).map((row) => ({
      id: String(row.id || ""),
  ...
  if (modules.includes("plans")) {
    ...
    const schedules = (scheduleRows as any[]).map((row) => ({
      id: String(row.id || ""),
      type: "Schedule",
    ...
    const productions = (productionRows as any[]).map((row) => ({
      id: String(row.id || ""),
      type: "Job",
  ...
  if (modules.includes("dispatch")) {
    ...
    details.dispatch = (rows as any[]).map((row) => ({
      id: String(row.id || ""),
  ...
  if (modules.includes("billing")) {
    ...
    details.billing = (rows as any[]).map((row) => ({
      id: String(row.id || ""),
      invoiceNo: String(row.invoiceNo || "-"),
...
async function applyGlobalItemTransfer(
  db: any,
  database: string,
  user: AuthUser,
  source: GlobalItemRenameSource,
  ...
  const orderColumns = await getExistingColumnNames(db as mysql.Pool, database, "orders");
  const scheduleColumns = await getExistingColumnNames(db as mysql.Pool, database, "orders_schedule");
  const productionColumns = await getExistingColumnNames(db as mysql.Pool, database, "productions");
  const dispatchColumns = await getExistingColumnNames(db as mysql.Pool, database, "dispatch_plans");
  const invoiceColumns = await getExistingColumnNames(db as mysql.Pool, database, "invoice_line_items");
  ...
  const preMatchedOrderIds = (preOrderRows as any[]).map((row) => String(row.id || "")).filter(Boolean);

  let preMatchedScheduleIds: string[] = [];
  if (modules.includes("plans") && preMatchedOrderIds.length > 0) {
    ...
    preMatchedScheduleIds = (scheduleRows as any[]).map((row) => String(row.id || "")).filter(Boolean);
  }

  let preMatchedDispatchIds: string[] = [];
  if (modules.includes("dispatch")) {
    ...
    preMatchedDispatchIds = (dispatchRows as any[]).map((row) => String(row.id || "")).filter(Boolean);
  }

  if (modules.includes("orders") && orderColumns.has("itemId")) {
    const updateParts = ["`itemId` = ?", "`itemSource` = ?"];
    const params: any[] = [toItem.id, source];
    ...
    counts.orders = Number((result as any)?.affectedRows || 0);
  }
  ...
  if (modules.includes("plans")) {
    ...
    if (
      scheduleColumns.has("id") &&
      preMatchedScheduleIds.length > 0 &&
      (scheduleColumns.has("updatedBy") || scheduleColumns.has("updateTimestamp"))
    ) {
      const updateParts: string[] = [];
      const params: any[] = [];
      ...
      schedules = Number((result as any)?.affectedRows || 0);
    }
    ...
    if (productionColumns.has("itemId")) {
      ...
      productions = Number((result as any)?.affectedRows || 0);
    }
  ...
  if (
    ...
    dispatchColumns.has("id") &&
    preMatchedDispatchIds.length > 0 &&
    (dispatchColumns.has("updatedBy") || dispatchColumns.has("updateTimestamp"))
  ) {
    const updateParts: string[] = [];
    const params: any[] = [];
    ...
    counts.dispatch = Number((result as any)?.affectedRows || 0);
  }
  ...
  if (modules.includes("billing") && invoiceColumns.has("itemId")) {
    ...
    counts.billing = Number((result as any)?.affectedRows || 0);
  }

  return { counts, details };
}

async function validateGlobalItemTransferRequest(req: express.Request, res: express.Response) {
  const user = await getRequestUser(req);
  if (!user) {
