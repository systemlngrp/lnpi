...
export function PendingLoading() {
  ...
  const handleSubmitLoading = async () => {
    ...
    try {
      // 1. Create Loading Slip
      const newSlip: LoadingSlip = {
        id: crypto.randomUUID(),
        slipNo: "", // Will be generated by server
              ...
              {expandedTrucks.has(truck.truckId) && (
                <div className="p-4 space-y-4">
                  {truck.items.map((item) => (
                    <div key={item.itemId} className="border border-slate-200 rounded overflow-hidden">
                      <div className="bg-slate-50 px-4 py-2 flex items-center justify-between border-b border-slate-200">
                        <div className="flex items-center gap-2">
                          <Package size={16} className="text-slate-500" />
                          <span className="font-bold text-black uppercase text-sm">{item.itemName}</span>
                        </div>
                        <button 
                          onClick={() => handleOpenLoad(truck.truckId, item.itemId, item.plans)}
                          className="bg-indigo-600 text-white px-3 py-1 rounded text-xs font-bold hover:bg-indigo-700 transition shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-y-[2px]"
                        >
                          LOAD
                        </button>
                      </div>
                      ...
                      <tr key={p.id}>
                        ...
                        <td className="px-4 py-3 text-right">
                          <input 
                            ...
                            max={p.pendingQty}
                            min={0}
                            className="w-24 px-2 py-1 border-2 border-indigo-600 rounded text-right focus:outline-none focus:ring-1 focus:ring-indigo-600"
                          />
                        </td>
                      </tr>
                    ))}
