...
export function StandaloneLoadingMaster({ source }: StandaloneLoadingMasterProps) {
  ...
  const processedSlips = useMemo(() => {
    ...
    return loadingSlips
      .map((slip) => {
        ...
        const firstPlan = planMap.get(String(relevantLines[0].dispatchPlanId || "").trim());
        ...
        return {
          slip,
          relevantLines,
          itemName: item?.name || String(firstProduction?.itemId || "-"),
          companyName: firstProduction?.companyName || item?.companyName || "-",
      ...
      .filter((row) => {
        ...
        const haystack = [row.slip.slipNo, row.itemName, row.companyName, row.jobNo, row.truckNo].join(" ").toLowerCase();
  ...
  }, [items, loadingSlips, planMap, productionMap, searchTerm, source, trucks]);
  ...
  const cancelSlip = async (slipId: string) => {
    ...
    if (!slip || slip.status === "Cancelled") return;
        ...
        <table className="min-w-full divide-y divide-black border-collapse">
          ...
          <tbody>
            {paginatedItems.length === 0 ? (
              <tr>
                <td colSpan={9} className="px-6 py-8 text-center text-black font-medium">No loading slips found.</td>
              </tr>
            ) : (
