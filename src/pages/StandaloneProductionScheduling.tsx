...
export function StandaloneProductionScheduling({ source }: StandaloneProductionSchedulingProps) {
  ...
  const openScheduler = (job: JobRow) => {
    ...
    setPlannedQty(job.plannedQty === undefined || job.plannedQty === null ? String(job.qty || "") : String(job.plannedQty || ""));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1.4fr)_420px] gap-6">
                ...
                paginatedItems.map((job) => {
                  ...
                  return (
                    <tr key={`${job.jobSource}:${job.id}`} className={isSelected ? "border-t border-black bg-indigo-50" : "border-t border-black"}>
                      ...
                      <td className="px-3 py-2 text-sm text-right">{formatNumber(job.plannedQty || job.qty)}</td>
        ...
        <div className="bg-white border border-black rounded shadow-sm p-4 space-y-4 h-fit">
          ...
          {selectedJob ? (
            ...
            <div className="rounded border border-dashed border-black p-4 text-sm font-medium text-slate-600">
              Select a job from the list to set Scheduled Date, Shift, Methodology, and Planned Qty.
            </div>
          )}
        </div>
      </div>
