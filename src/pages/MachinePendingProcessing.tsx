...
import { Hammer, Search, ChevronRight, ChevronDown } from "lucide-react";
...
export function MachinePendingProcessing() {
  ...
  const machineGroups = useMemo(() => {
    ...
    // Initialize groups for all active machines
    machines.forEach(m => {
  ...
  }, [productions, items, machines, processing, mandatoryMachinesMapping, searchTerm]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-black pb-4">
        <h2 className="text-xl font-bold text-black uppercase tracking-tight">Machine-wise Pending Processing</h2>
        ...
        {machineGroups.length === 0 ? (
          ...
          machineGroups.map((group) => (
            <div key={group.machineId} className="bg-white border border-black rounded shadow-sm overflow-hidden">
              ...
              {expandedMachines.has(group.machineId) && (
                <div className="p-4">
