...
export function StandaloneProductionExecution({ source }: Props) {
  ...
  const pendingJobs = useMemo(() => {
    ...
    return jobs
      ...
      .filter((job) => {
        ...
        const haystack = [job.jobSource, job.transactionNo, job.erpCode, job.sequence, job.shift, job.scheduledDate, item?.name].join(" ").toLowerCase();
  ...
  const saveProduction = async () => {
    ...
    const updateJobs = (prev: Production[]) => prev.map((job) => job.id === selectedJob.id ? { ...job, jobCompletionTimeOutput: completionTime, productionOutputQty: nextOutput, updatedBy: "System User", updateTimestamp: timestamp } : job);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-black rounded shadow-sm overflow-auto">
        <table className="min-w-[1360px] w-full divide-y divide-black border-collapse">
          ...
          <tbody>
            {paginatedItems.length === 0 ? (
              <tr><td colSpan={7} className="px-6 py-8 text-center text-black font-medium">No jobs pending production.</td></tr>
      ...
      {selectedJob ? (
        <div className="bg-white border border-black rounded shadow-sm p-4 max-w-md mx-auto w-full">
          <h3 className="text-sm font-black uppercase tracking-wide text-black mb-4">Production</h3>
          ...
          <div className="text-xs font-black uppercase text-slate-500">Job Completion Time</div>
          ...
          <div className="mt-4 text-xs font-black uppercase text-slate-500">Output</div>
          <input type="number" min={0} step="any" value={outputQty} onChange={(e) => setOutputQty(e.target.value)} className="mt-1 w-full border-2 border-black rounded p-2 text-black focus:outline-none focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600" />
          <div className="mt-4 flex gap-3">
            <button type="button" onClick={() => void saveProduction()} className="rounded border border-black bg-emerald-600 px-4 py-2 text-sm font-bold uppercase text-white hover:bg-emerald-700">Save</button>
            <button type="button" onClick={() => { setSelectedJobKey(""); setCompletionTime(""); setOutputQty(""); }} className="rounded border border-black bg-white px-4 py-2 text-sm font-bold uppercase text-black hover:bg-slate-50">Cancel</button>
          </div>
        </div>
      ) : null}
