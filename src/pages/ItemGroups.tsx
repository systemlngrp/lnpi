...
export function ItemGroups() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const isDuplicate = groups.some(g => 
      g.name.toLowerCase() === name.trim().toLowerCase() && g.id !== editingId
    );

    if (isDuplicate) {
      alert("An item group with this name already exists.");
      return;
    }
    ...
    setTimeout(() => {
      ...
      if (editingId) {
        setGroups(groups.map(g => g.id === editingId ? { ...g, name: name.trim(), ...audit } : g));
      } else {
        setGroups([...groups, { id: crypto.randomUUID(), name: name.trim(), ...audit }]);
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center pb-4 border-b border-black">
        <h2 className="text-xl font-bold text-black uppercase tracking-tight">Item Groups Master</h2>
        {!isFormOpen && (
          <button
      ...
      {isFormOpen ? (
        <div className="bg-white p-6 rounded shadow-sm border border-black max-w-xl">
          <h3 className="text-lg font-bold text-black mb-6 uppercase">
            {editingId ? "Edit Item Group" : "Create Item Group"}
          </h3>
                ...
                <div key={group.id} className="bg-white border-2 border-black p-4 space-y-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] rounded">
                    <div className="flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-2">
                             <button onClick={() => { setName(group.name); setEditingId(group.id); setIsFormOpen(true); }} className="text-indigo-600 hover:text-indigo-900 font-bold"><Edit size={16} /></button>
                             <button 
        ...
        <table className="hidden md:table min-w-full divide-y divide-black border-collapse border border-black">
              ...
              <tbody className="divide-y divide-black bg-white">
                {filteredGroups.length === 0 ? (
                  <tr>
                    <td colSpan={2} className="px-6 py-8 text-center text-black font-medium">No item groups found.</td>
                  </tr>
                ) : (
                  filteredGroups.sort((a, b) => {
                    const timeA = a.updateTimestamp ? new Date(a.updateTimestamp).getTime() : 0;
                    ...
                    <tr key={group.id} className="hover:bg-slate-50 divide-x divide-black">
                      ...
                      <td className="px-6 py-4 text-right text-sm font-medium border border-black whitespace-nowrap">
                        <button onClick={() => { setName(group.name); setEditingId(group.id); setIsFormOpen(true); }} className="text-indigo-600 hover:text-indigo-900 mr-4 font-bold inline-flex items-center">
