...
export function MaterialIssueMaster() {
  ...
  return (
    <div className="space-y-6">
      ...
      <TableControls searchTerm={searchTerm} onSearchChange={setSearchTerm} placeholder="Search issue no, date, type, job..." />
                ...
                filtered.map((row) => (
                  <tr key={row.id} className="divide-x divide-black hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-sm">
                      {isWithoutJobIssue(row.issueType) ? "Without Job" : row.issueType}
                    </td>
